# medusa_jittor_all/train/train_head_only.py
import os
import json
import argparse
import socket
...
from medusa_jittor_all.model.medusa_head import MedusaHead
...
def masked_cross_entropy_and_top1(logits, labels, ignore_id: int):
    ...
    logp = nn.log_softmax(logits.float32(), dim=1)  # [N,V]
        ...
        for ep in range(args.epochs):
            ...
            for item in samples:
                ...
                if np.isnan(hidden).any() or np.isinf(hidden).any():
                    print(f"[Trainer] Warning: Received hidden states with NaN/Inf at step {step}")
                    continue
                
                # Cast to float32 for stability
                hidden_jt = jt.array(hidden).float32()
