...
# -*- coding: utf-8 -*-
"""
MT-Bench throughput benchmark (single file):
- baseline: HF/torch generate
- medusa: Jedusa (jittor) medusa_generate
- jt_baseline: jittor greedy decode (no medusa heads)

Features:
- --run baseline/medusa/jt_baseline/both
- saves:
  ...
  * FastChat judge answer jsonl: --baseline_answer_file / --medusa_answer_file / --jt_answer_file

Key fix for "medusa tokens drop":
- Medusa prompt truncation reserves SAME output budget as baseline:
...
def build_prompt(conv_name: str, user_messages: List[str], assistant_messages: List[str]):
    if get_conv_template is None:
        raise RuntimeError(
            "fastchat is required for MT-Bench prompt templates, but import failed.\n"
            f"Import error: {_FASTCHAT_IMPORT_ERR}\n"
            "Fix: pip install fschat (or install fastchat in your env)."
        )
...
def gen_jt_baseline(
    ...
    for step in range(max_steps):
        next_id = _jt_argmax_indices(logits[:, -1, :], dim=-1).cast(jt.int32)
...
def replace_run(argv: List[str], new_run: str) -> List[str]:
    return replace_or_add_arg(argv, "--run", new_run)
...
def main():
    ...
    # Parallel Dispatcher
    # ==========================================================
    if args.num_gpus > 1 and args.run != "both":
        import math
        
        total_q = len(qs)
        chunk_size = math.ceil(total_q / args.num_gpus)
        
        procs = []
        base_argv = sys.argv[1:]
        base_argv = replace_or_add_arg(base_argv, "--num_gpus", "1")
        
        abs_start = args.start
        
        # Track output files for merging
        part_out_jsons = []
        part_detail_jsonls = []
        part_answer_files = defaultdict(list)
        
        for i in range(args.num_gpus):
            sub_start = abs_start + i * chunk_size
            sub_limit = min(chunk_size, total_q - i * chunk_size)
            
            if sub_limit <= 0:
                break
                
            cmd_argv = list(base_argv)
            cmd_argv = replace_or_add_arg(cmd_argv, "--start", str(sub_start))
            cmd_argv = replace_or_add_arg(cmd_argv, "--limit", str(sub_limit))
            
            if args.out_json:
                p = Path(args.out_json)
                part_json = str(p.with_suffix(f".part{i}{p.suffix}"))
                cmd_argv = replace_or_add_arg(cmd_argv, "--out_json", part_json)
                part_out_jsons.append(part_json)
            
            if args.detail_jsonl:
                p = Path(args.detail_jsonl)
                part_jsonl = str(p.with_suffix(f".part{i}{p.suffix}"))
                cmd_argv = replace_or_add_arg(cmd_argv, "--detail_jsonl", part_jsonl)
                part_detail_jsonls.append(part_jsonl)
                
            for ans_arg in ["--baseline_answer_file", "--medusa_answer_file", "--jt_answer_file"]:
                val = getattr(args, ans_arg[2:])
                if val:
                    p = Path(val)
                    part_ans = str(p.with_suffix(f".part{i}{p.suffix}"))
                    cmd_argv = replace_or_add_arg(cmd_argv, ans_arg, part_ans)
                    part_answer_files[ans_arg].append(part_ans)
            
            if args.log_file:
                p = Path(args.log_file)
                part_log = str(p.with_suffix(f".part{i}{p.suffix}"))
                cmd_argv = replace_or_add_arg(cmd_argv, "--log_file", part_log)

            env = os.environ.copy()
            env["CUDA_VISIBLE_DEVICES"] = str(i)
            
            print(f"[Dispatcher] Launching worker {i} on GPU {i}: start={sub_start} limit={sub_limit}")
            p = subprocess.Popen([sys.executable, os.path.abspath(__file__)] + cmd_argv, env=env)
            procs.append(p)
        ...
        print("[Dispatcher] Merging results...")
        
        # Merge JSON
        if args.out_json and part_out_jsons:
            merged = {
                "run": args.run,
                "conv": args.conv,
                "questions": len(qs),
            ...
            final_res = {
                ...
                "per_category": defaultdict(lambda: {"n_questions": 0, "time_s": 0.0, "tokens": 0, "tokens_per_second": 0.0})
            }
            
            max_worker_time = 0.0
            model_meta = {}
            
            for pjson in part_out_jsons:
                if not os.path.exists(pjson): continue
                data = load_json(pjson)
                if "results" not in data or args.run not in data["results"]: continue
                
                res = data["results"][args.run]
                final_res["n_questions"] += res["n_questions"]
                final_res["total_tokens"] += res["total_tokens"]
                max_worker_time = max(max_worker_time, res["total_time_s"])
                
                for k in ["model", "max_ctx", "kv_cache_len"]:
                    if k in res: model_meta[k] = res[k]
                
                for cat, cdata in res["per_category"].items():
                    dst = final_res["per_category"][cat]
                    dst["n_questions"] += cdata["n_questions"]
                    dst["time_s"] += cdata["time_s"]
                    dst["tokens"] += cdata["tokens"]
            
            final_res["total_time_s"] = max_worker_time
            final_res["tokens_per_second"] = final_res["total_tokens"] / max(max_worker_time, 1e-9)
            final_res.update(model_meta)
            
            for cat, dst in final_res["per_category"].items():
                dst["tokens_per_second"] = dst["tokens"] / max(dst["time_s"], 1e-9)
            
            merged["results"][args.run] = final_res
            save_json(merged, args.out_json)
            
            for f in part_out_jsons:
                try: os.remove(f)
                except: pass

        # Merge JSONL
        if args.detail_jsonl and part_detail_jsonls:
            with open(args.detail_jsonl, "w", encoding="utf-8") as fout:
                for f in part_detail_jsonls:
                    if os.path.exists(f):
        ...
        # Merge Answers
        for ans_arg, parts in part_answer_files.items():
            final_path = getattr(args, ans_arg[2:])
            if final_path and parts:
    ...
    # ==========================================================
    # --run both: run medusa then jt_baseline as subprocess
    ...
    if args.run == "both":
        ...
        stage_out_jt_baseline = None
        if args.out_json:
            p = Path(args.out_json)
            stage_out_medusa = str(p.with_suffix(p.suffix + ".medusa.json"))
            stage_out_jt_baseline = str(p.with_suffix(p.suffix + ".jt_baseline.json"))
        ...
        argv_b = replace_run(argv, "jt_baseline")
        if stage_out_jt_baseline:
            argv_b = replace_or_add_arg(argv_b, "--out_json", stage_out_jt_baseline)
        r2 = subprocess.run(base_cmd + argv_b, env=env)
        if r2.returncode != 0:
            raise SystemExit(r2.returncode)

        if args.out_json and stage_out_medusa and stage_out_jt_baseline:
            try:
                jm = load_json(stage_out_medusa)
                jb = load_json(stage_out_jt_baseline)
                merged = {
                    "run": "both",
                    "conv": args.conv,
                    "questions": len(qs),
                ...
                if isinstance(jb, dict) and "results" in jb and "jt_baseline" in jb["results"]:
                    merged["results"]["jt_baseline"] = jb["results"]["jt_baseline"]
    ...
    if args.run == "medusa":
        ...
        if hasattr(MedusaModel, "from_pretrained") and callable(getattr(MedusaModel, "from_pretrained")):
            ...
            # ✅ Fix: If medusa_model path lacks config.json (e.g. just weights), 
            # try to load config from baseline_model if provided.
            try:
                medusa_model = MedusaModel.from_pretrained(args.medusa_model, **kw)
            except OSError as e:
                if "config.json" in str(e) and args.baseline_model:
                    print(f"[Medusa] Config not found in {args.medusa_model}, trying baseline config from {args.baseline_model}...")
                    from transformers import AutoConfig
                    config = AutoConfig.from_pretrained(args.baseline_model)
                    kw["config"] = config
                    medusa_model = MedusaModel.from_pretrained(args.medusa_model, **kw)
                else:
                    raise e
        ...
        try:
            if hasattr(medusa_model, "config") and getattr(medusa_model.config, "max_position_embeddings", None):
                ...
                hasattr(medusa_model, "base_model")
                and hasattr(medusa_model.base_model, "config")
                and getattr(medusa_model.base_model.config, "max_position_embeddings", None)
            ):
        ...
        if hasattr(MedusaModel, "from_pretrained") and callable(getattr(MedusaModel, "from_pretrained")):
            ...
            # ✅ Fix: Same fallback for jt_baseline
            try:
                jt_wrapper = MedusaModel.from_pretrained(args.medusa_model, **kw)
            except OSError as e:
                if "config.json" in str(e) and args.baseline_model:
                    print(f"[jt-baseline] Config not found in {args.medusa_model}, trying baseline config from {args.baseline_model}...")
                    from transformers import AutoConfig
                    config = AutoConfig.from_pretrained(args.baseline_model)
                    kw["config"] = config
                    jt_wrapper = MedusaModel.from_pretrained(args.medusa_model, **kw)
